...
<body class="background-all text-warning">
    ...
    <div class="container">
        ...
        <table class="table table-hover text-warning">
            ...
            <tbody data-click-to-select="true" class="table-dark">
                ...
                ?>
                            <tr>
                                <td><?= $i; ?></td>
                                <td><a href="user.php?ID_user=<?= $row['ID_user']; ?>" class="text-warning"><?= $row['ID_user']; ?></a></td>
                                <td><?= $row['user_name']; ?></td>
        ...
        <ul class="pagination">
                <li class="nav-item">                    
                    <a class="nav-link" href="ranking.php?page=1" aria-label="Previous">
                    <button type="button" class="btn text-warning">&laquo;</button>
                    </a>
                </li>
                <?php for($i = 1; $i <= $total_page; $i++){ ?>
                    <li class="nav-item">
                    <a class="nav-link" href="ranking.php?page=<?php echo $i; ?>">
                        <?php if($i == $page){ ?>
                ...
                <?php } ?>
                <li class="nav-item">
                    <a class="nav-link" href="ranking.php?page=<?php echo $total_page;?>" aria-label="Next">
